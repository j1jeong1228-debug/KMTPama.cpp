...
llama_model_gemma4::graph_mtp::graph_mtp(
    ...
    tok_e = ggml_scale(ctx0, tok_e, std::sqrt((float) target.hparams.n_embd));
    ...
    if (std::getenv("LLAMA_GEMMA4_MTP_GRAPH_MARGIN_MIN") != nullptr ||
            std::getenv("LLAMA_GEMMA4_MTP_LONGCTX_GRAPH_MARGIN_MIN") != nullptr) {
        ggml_tensor * top2 = ggml_top_k(ctx0, cur, 2);
        cb(top2, "result_top2", -1);
        ggml_tensor * logits_rows = ggml_reshape_2d(ctx0, cur, 1, n_vocab);
        ggml_tensor * top2_logits = ggml_get_rows(ctx0, logits_rows, top2);
        top2_logits = ggml_reshape_1d(ctx0, top2_logits, 2);
        cb(top2_logits, "result_top2_logits", -1);
        res->t_argmax_prob = top2_logits;
        ggml_build_forward_expand(gf, top2_logits);
    }
...
void llama_model_gemma4_assistant::load_arch_tensors(llama_model_loader &) {
    ...
    if (hparams.n_embd_backbone == 0) {
        throw std::runtime_error("gemma4-assistant: missing backbone hidden size");
    }
    ...
    assist_pre_proj = create_tensor(
            tn(LLM_TENSOR_ASSIST_PRE_PROJ, "weight"),
            {2 * n_embd_backbone, n_embd}, TENSOR_NOT_REQUIRED);
    if (assist_pre_proj == nullptr) {
        assist_pre_proj = create_tensor(
                tn(LLM_TENSOR_ASSIST_PRE_PROJ_DOTTED, "weight"),
                {2 * n_embd_backbone, n_embd}, TENSOR_NOT_REQUIRED);
    }
    if (assist_pre_proj == nullptr) {
        assist_pre_proj = create_tensor(
                tn(LLM_TENSOR_ASSIST_PRE_PROJ_NEXTN, "weight"),
                {2 * n_embd_backbone, n_embd}, 0);
    }

    assist_post_proj = create_tensor(
            tn(LLM_TENSOR_ASSIST_POST_PROJ, "weight"),
            {n_embd, n_embd_backbone}, TENSOR_NOT_REQUIRED);
    if (assist_post_proj == nullptr) {
        assist_post_proj = create_tensor(
                tn(LLM_TENSOR_ASSIST_POST_PROJ_DOTTED, "weight"),
                {n_embd, n_embd_backbone}, TENSOR_NOT_REQUIRED);
    }
    if (assist_post_proj == nullptr) {
        assist_post_proj = create_tensor(
                tn(LLM_TENSOR_ASSIST_POST_PROJ_NEXTN, "weight"),
                {n_embd, n_embd_backbone}, 0);
    }

    if (hparams.use_ordered_embeddings) {
        if (n_centroids == 0) {
            throw std::runtime_error("gemma4-assistant: ordered embeddings enabled with zero centroids");
        }
        assist_embed_centroids = create_tensor(
                tn(LLM_TENSOR_ASSIST_EMBED_CENTROIDS, "weight"),
                {n_embd, n_centroids}, 0);
        assist_token_ordering = create_tensor(
                tn(LLM_TENSOR_ASSIST_TOKEN_ORDERING, "weight"),
                {n_vocab}, 0);
    }
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        if (!hparams.is_swa(i)) {
            layer.rope_freqs = create_tensor(
                    ...
                    {n_embd_head / 2}, TENSOR_NOT_REQUIRED | rope_freqs_flag);
...
llama_model_gemma4_assistant::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    GGML_UNUSED(model);
    GGML_ABORT(
        "gemma4-assistant: standalone decode is unsupported; load this GGUF "
        "as an attached MTP assistant for a compatible Gemma4 target model");
}
...
ggml_tensor * llama_model_gemma4_assistant::graph::build_masked_embedding_logits(ggml_tensor * hidden, ggml_tensor * lm_head_w) {
    ...
    GGML_ABORT("gemma4-assistant: masked embedding logits are implemented in the attached-MTP graph");
}
